...
# CfC / MLP backbones for SECOM (static process-sensor feature vectors).
...
class SecomMLP(MammothBackbone):
    def __init__(self, input_size=562, num_classes=6, hidden_size=128):
        super().__init__()
...
class SecomCfC(MammothBackbone):
    """CfC backbone for SECOM, treating each row as a length-1 sequence."""

    def __init__(self, input_size=562, num_classes=6, hidden_size=64, use_ncp_wiring=True):
        super().__init__()
...
@register_backbone('secommlp')
def secommlp(input_size: int = 562, num_classes: int = 6, hidden_size: int = 128):
    return SecomMLP(input_size=input_size, num_classes=num_classes, hidden_size=hidden_size)


@register_backbone('secomcfc')
def secomcfc(input_size: int = 562, num_classes: int = 6, hidden_size: int = 64, use_ncp_wiring: bool = True):
    return SecomCfC(input_size=input_size, num_classes=num_classes,
